...
export async function GET() {
  try {
    ...
    const [logs, freshness, manualNotifications, audit] = await Promise.all([
      getCollectorLogs(),
      getCollectorFreshness().catch(() => ({ lastSuccessfulCheckAt: null })),
      listManualBroadcastHistory(10).catch(() => []),
      suspensionStore.listAudit(5, 0),
    ]);
    return Response.json({ session, sources, health: deriveCollectorHealth(sources, logs, freshness), logs, records, audit: audit.entries, auditTotal: audit.total, manualNotifications, registries: { lgus: Object.values(NCR_LGUS).map(({ id, name }) => ({ id, name })), schools: NCR_SCHOOLS.map(({ id, name, campusName, lguId, sector, levelsOffered }) => ({ id, name, campusName, lguId, sector, levelsOffered })) } }, { headers: { "Cache-Control": "no-store, private" } });
