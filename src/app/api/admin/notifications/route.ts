...
export async function POST(request: NextRequest) {
  try { await requireAdminMutation(request); const result = await sendAdminNotification(schema.parse(await readBoundedJson(request, 4_096)) as never); return Response.json(result, { status: result.created ? 201 : 200, headers: { "Cache-Control": "no-store, private" } }); }
