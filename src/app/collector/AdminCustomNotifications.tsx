...
type Confirmation = { recipientCount: number | null };
...
export function AdminCustomNotifications({ csrfToken, history, onSent }: { csrfToken: string; history: ManualBroadcastHistoryEntry[]; onSent: () => Promise<void> }) {
  const [draft, setDraft] = useState<Draft>(freshDraft); const [loaded, setLoaded] = useState(false); const [busy, setBusy] = useState(false); const [error, setError] = useState(""); const [confirmation, setConfirmation] = useState<Confirmation | null>(null); const cancelRef = useRef<HTMLButtonElement>(null); const selected = useMemo(() => new Set(draft.targetLguIds), [draft.targetLguIds]);
  ...
  async function prepare(event: React.FormEvent) {
    ...
    setBusy(true); try { const preview = await request("/api/admin/notifications/preview") as { available: boolean; recipientCount: number | null }; setConfirmation({ recipientCount: preview.available ? preview.recipientCount : null }); } catch (reason) { setError(reason instanceof Error ? reason.message : "Could not calculate recipients."); } finally { setBusy(false); }
  }
  async function send() {
