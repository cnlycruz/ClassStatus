...
export function SuspensionAlerts({ selectedLguId }: { selectedLguId?: LGUId | null }) {
  ...
  const canManageLocations = ready && !unavailable && permission !== "denied";
  return <>
    ...
    {open && createPortal(<div className="fixed inset-0 z-[100] flex items-center justify-center overflow-hidden bg-slate-950/60 px-[max(0.75rem,env(safe-area-inset-left))] pb-[max(0.75rem,env(safe-area-inset-bottom))] pt-[max(0.75rem,env(safe-area-inset-top))]"><div ref={dialogRef} tabIndex={-1} role="dialog" aria-modal="true" aria-labelledby="suspension-alerts-title" className="flex max-h-[calc(100dvh-1.5rem-env(safe-area-inset-top)-env(safe-area-inset-bottom))] w-full max-w-lg flex-col overflow-hidden rounded-2xl bg-white shadow-2xl outline-none dark:bg-slate-900">
      <div className="flex shrink-0 items-start justify-between gap-3 px-4 pb-3 pt-4 sm:px-5 sm:pt-5"><div><h2 id="suspension-alerts-title" className="flex items-center gap-2 text-lg font-bold text-slate-950 dark:text-white">{enabled ? <Bell className="h-5 w-5 text-blue-600" aria-hidden="true" /> : <BellOff className="h-5 w-5 text-slate-500" aria-hidden="true" />}Suspension alerts</h2><p className="mt-1 text-sm text-slate-600 dark:text-slate-300">Get notified when a verified class suspension is published for the places you care about.</p></div><button type="button" onClick={() => setOpen(false)} aria-label="Close suspension alert settings" className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl text-slate-500 hover:bg-slate-100 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-500 dark:hover:bg-slate-800"><X className="h-5 w-5" /></button></div>
      <div className="min-h-0 flex-1 overflow-y-auto px-4 pb-4 sm:px-5">
        {!ready ? <p className="text-sm text-slate-500">Checking notification support…</p> : unavailable ? <p className="rounded-xl border border-slate-200 bg-slate-50 p-3 text-sm text-slate-700 dark:border-slate-800 dark:bg-slate-950 dark:text-slate-300">{unavailable}</p> : permission === "denied" ? <p className="rounded-xl border border-amber-200 bg-amber-50 p-3 text-sm text-amber-950 dark:border-amber-900 dark:bg-amber-950 dark:text-amber-100">Notifications are blocked in your browser. Allow notifications in your browser settings to enable suspension alerts.</p> : <>
