...
export function projectPublicSuspension(record: SuspensionRecord) {
  ...
  return {
    ...
    publication: manual ? "Manually verified by Class Status Admin" : record.publicationProvenance?.publicLabel || "Published from approved Tier 3 media evidence",
    publicationProvenance: manual ? { type: "manual-admin", publicLabel: "Manually verified by Class Status Admin" } : { type: "automatic-collector", publicLabel: record.publicationProvenance?.publicLabel || "Published from approved Tier 3 media evidence" },
