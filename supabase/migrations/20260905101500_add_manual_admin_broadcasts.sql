-- Manual broadcasts share the existing private events/deliveries outbox while
-- remaining distinguishable from collector publication events.
alter table public.classstatus_notification_events
  add column event_type text not null default 'automatic' check (event_type in ('automatic', 'manual')),
  add column request_key uuid,
  add column title text,
  add column message text,
  add column recipient_mode text check (recipient_mode in ('all', 'selected-lgus')),
  add column target_lgu_ids text[] not null default '{}',
  add column recipient_count integer check (recipient_count >= 0);
alter table public.classstatus_notification_events drop constraint classstatus_notification_events_fingerprint_check;
alter table public.classstatus_notification_events add constraint classstatus_notification_events_fingerprint_check check (fingerprint ~ '^(?:v1:[0-9a-f]{64}|manual:[0-9a-f-]{36})$');
alter table public.classstatus_notification_events drop constraint classstatus_notification_events_kind_check;
alter table public.classstatus_notification_events add constraint classstatus_notification_events_kind_check check (kind in ('initial', 'update', 'manual'));
create unique index classstatus_notification_events_manual_request_key_idx
  on public.classstatus_notification_events (deployment_namespace, request_key) where request_key is not null;
...
create function public.classstatus_preview_worker_manual_notification_store(p_payload text, p_issued_at bigint, p_nonce uuid, p_signature text) returns jsonb language plpgsql security definer set search_path = '' as $$ declare payload jsonb; begin payload := classstatus_private.verify_preview_collector_capability('logs.append', p_payload, p_issued_at, p_nonce, p_signature, 8192); perform set_config('classstatus.manual_notification_worker', 'preview', true); return classstatus_private.manual_notification_store('preview', payload); end $$;
create function public.classstatus_production_worker_manual_notification_store(p_payload text, p_issued_at bigint, p_nonce uuid, p_signature text) returns jsonb language plpgsql security definer set search_path = '' as $$ declare payload jsonb; begin payload := classstatus_private.verify_production_collector_capability('logs.append', p_payload, p_issued_at, p_nonce, p_signature, 8192); perform set_config('classstatus.manual_notification_worker', 'production', true); return classstatus_private.manual_notification_store('production', payload); end $$;
revoke execute on function public.classstatus_preview_worker_manual_notification_store(text,bigint,uuid,text) from public, anon, authenticated, service_role;
revoke execute on function public.classstatus_production_worker_manual_notification_store(text,bigint,uuid,text) from public, anon, authenticated, service_role;
grant execute on function public.classstatus_preview_worker_manual_notification_store(text,bigint,uuid,text) to anon;
grant execute on function public.classstatus_production_worker_manual_notification_store(text,bigint,uuid,text) to anon;
