...
describe("persistent public navbar contracts", () => {
  ...
  it("keeps startup branding scoped to an initial public shell mount", () => {
    ...
    expect(splash).toContain("NCR_GEO_PATHS");
    expect(splash).toContain("NCR_MAP_BASE_VIEWBOX");
    expect(splash).toContain("STARTUP_LGU_ORDER");
    expect(splash).toContain("reducedMotion ? 300 : 3_000");
    expect(styles).toContain("@keyframes startup-lgu-assemble");
    expect(styles).toContain("@keyframes startup-ncr-complete");
    expect(styles).toContain("@keyframes startup-splash-dismiss");
    expect(styles).toContain("safe-area-inset-top");
    expect(styles).toContain(".startup-ncr-map");
    expect(styles).toContain(".startup-lgu");
